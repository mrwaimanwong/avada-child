...
function callrail_admin_notice() {
	...
	if( $is_plugins_page && !$api_key && function_exists( "admin_url" ) )
		echo '<div class="error"><p><strong>' . sprintf( __('<a href="%s">Enter your WordPress Plugin Key</a> to enable dynamic tracking number insertion.', 'callrail' ), admin_url( 'options-general.php?page=callrail' ) ) . '</strong></p></div>';
}
